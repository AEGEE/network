...
exports.createBoard = async (req, res) => {
    ...
    const body_name = (await core.fetchBody(req.body, req.headers['x-auth-token'])).body_name;
    ...
    await sequelize.transaction(async (t) => {
        const board = await Board.create(req.body, { transaction: t });

        await mailer.sendMail({
            to: config.new_board_notifications,
            subject: `A new board was added for ${body_name}`,
