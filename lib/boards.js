...
// exports.listAllCurrentBoards = async (req, res) => {
//     if (!req.permissions.view_board) {
//         return errors.makeForbiddenError(res, 'You are not allowed to list all current boards.');
//     }
...
exports.listMostRecentBoardsElected = async (req, res) => {
    ...
    if (req.query.ends) end_date = moment(req.query.ends, 'YYYY-MM-DD').endOf('day').toDate()

    const boards = await Board.findAll({
        where: {
            elected_date: { [Op.lte]: end_date },
        },
        group: 'body_id',
        attributes: [
            'body_id',
